    ...
    showDialog(
      ...
      builder: (context) => FormDialogLayout(
        ...
        content: Column(
          ...
          children: [
            TextField(controller: nameController, decoration: const InputDecoration(labelText: 'Nombre del Sprint', border: OutlineInputBorder())),
            const SizedBox(height: 12),
            TextField(controller: descController, maxLines: 3, decoration: const InputDecoration(labelText: 'Descripción', border: OutlineInputBorder())),
          ],
        ),
        onSave: () async {
          if (nameController.text.trim().isEmpty) return false;
          final newSprint = await _kanbanService.createSprint(widget.proyecto.id, nameController.text.trim(), descController.text.trim());
      ...
      builder: (context) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('Crea o edita los estados globales del tablero.', style: TextStyle(color: Theme.of(context).textTheme.bodySmall?.color)),
              const SizedBox(height: 16),
              ..._columns.map((col) => ListTile(
                leading: CircleAvatar(backgroundColor: Color(int.parse(col.color.replaceFirst('#', '0xFF'))), radius: 10),
                title: Text(col.nombre),
                trailing: IconButton(
                  icon: const Icon(Icons.edit, size: 20),
                  onPressed: () {
                    Navigator.pop(context); // close first modal
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setModalState) => FormDialogLayout(
          ...
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              TextField(controller: nameCtrl, decoration: const InputDecoration(labelText: 'Nombre del Estado', border: OutlineInputBorder())),
              const SizedBox(height: 16),
              const Align(alignment: Alignment.centerLeft, child: Text('Color Hexadecimal (Ej. #42A5F5)')),
              Padding(
                padding: const EdgeInsets.only(top: 8.0),
                child: Row(
                  children: [
                    CircleAvatar(backgroundColor: Color(int.parse(selectedHex.replaceFirst('#', '0xFF'))), radius: 15),
          ...
          onSave: () async {
            ...
            if (column == null) {
              return await _kanbanService.createColumn(widget.proyecto.id, nameCtrl.text.trim(), selectedHex, _columns.length + 1);
            } else {
              return await _kanbanService.updateColumn(column.id, nameCtrl.text.trim(), selectedHex);
            }
          },
          onSuccess: _loadBoard,
          ...
          if (_isSidebarExpanded) ...[
            Expanded(
              child: _isLoadingSprints
                  ...
                  : _hasSprintsError
                      ? ErrorState(message: 'Error al cargar sprints', onRetry: _loadSprints)
                      : _sprints.isEmpty
                          ? Center(child: Text('Sin sprints', style: TextStyle(color: Theme.of(context).textTheme.bodySmall?.color)))
                          : ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return ListTile(
                                  ...
                                  title: Text(sprint.nombre, style: TextStyle(fontWeight: isSelected ? FontWeight.bold : FontWeight.normal)),
                                  subtitle: Text(sprint.estado == 0 ? 'Planeado' : 'Activo'), // simple enum map
                                  selected: isSelected,
                                  onTap: () {
                        ...
                        return Tooltip(
                          ...
                          child: InkWell(
                            ...
                            child: Container(
                              ...
                              child: Icon(Icons.av_timer, color: isSelected ? Theme.of(context).colorScheme.primary : Theme.of(context).iconTheme.color),
                            ),
                          ),
                        );
                      },
  ...
  Widget _buildMainBoard() {
    if (_sprints.isEmpty && !_isLoadingSprints) {
      return const EmptyState(
        ...
        subtitle: 'Crea un sprint en la barra lateral para comenzar a organizar tareas.',
    ...
    if (_columns.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(Icons.dashboard_customize_outlined, size: 64, color: Theme.of(context).iconTheme.color),
            const SizedBox(height: 16),
            const Text(
            ...
            CustomButton(
              onPressed: () async {
                ...
                try {
                  await _kanbanService.initializeDefaultColumns(widget.proyecto.id);
                  await _loadBoard();
                } catch (e) {
                  setState(() => _isLoadingBoard = false);
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Error al inicializar tablero')),
                    );
                  } // UI hook
                }
              },
        ...
        Expanded(
          child: SingleChildScrollView(
            ...
            child: Row(
              ...
              children: _columns.map<Widget>((col) => _buildColumn(col)).toList(),
            ),
          ),
        ),
      ],
    ...
    return DragTarget<TaskModel>(
      ...
      onAcceptWithDetails: (details) async {
        ...
        if (success) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Error al mover tarea')));
        }
      },
      builder: (context, candidateData, rejectedData) {
        final isHovered = candidateData.isNotEmpty;
        return Container(
          width: 250, // Densidad más compacta (Desktop)
          margin: const EdgeInsets.only(right: 16),
          decoration: BoxDecoration(
            color: isHovered ? Theme.of(context).colorScheme.primaryContainer.withValues(alpha: 0.5) : Theme.of(context).colorScheme.surface,
            border: Border.all(color: isHovered ? Theme.of(context).colorScheme.primary : Theme.of(context).dividerColor),
            borderRadius: BorderRadius.circular(8),
          ),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.stretch,
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Draggable<TaskModel>(
                      ...
                      feedback: Material(
                        ...
                        child: SizedBox(
                          width: 234, // Aproximando margen interno
